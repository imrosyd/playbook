...
export default function ScatterChart({ state, width = 640, height = 400 }: ScatterChartProps) {
    ...
    useEffect(() => {
        ...
        // Using point scale for X to space out items like a bar chart / line chart
        ...
        if (state.params.confidenceLevel !== 'none') {
            ...
            errorBars.append('line')
                .attr('x1', (d) => x(d.label)!)
                .attr('x2', (d) => x(d.label)!)
                .attr('y1', (d) => y(d.value + marginOfError))
                .attr('y2', (d) => y(Math.max(0, d.value - marginOfError)))
                .attr('stroke', '#94a3b8')
                .attr('stroke-width', 1.5);

            errorBars.append('line')
                .attr('x1', (d) => x(d.label)! - 4)
                .attr('x2', (d) => x(d.label)! + 4)
                .attr('y1', (d) => y(d.value + marginOfError))
                .attr('y2', (d) => y(d.value + marginOfError))
                .attr('stroke', '#94a3b8')
                .attr('stroke-width', 1.5);
            ...
            errorBars.append('line')
                .attr('x1', (d) => x(d.label)! - 4)
                .attr('x2', (d) => x(d.label)! + 4)
                .attr('y1', (d) => y(Math.max(0, d.value - marginOfError)))
                .attr('y2', (d) => y(Math.max(0, d.value - marginOfError)))
                .attr('stroke', '#94a3b8')
                .attr('stroke-width', 1.5);
        ...
        g.selectAll('.dot')
            ...
            .attr('opacity', (_, i) => {
                if (highlighted.length === 0) return 0.8;
                return highlighted.includes(i) ? 0.9 : dimOpacity;
            })
            .attr('r', 0)
            .transition()
            .duration(500)
        ...
        if (state.params.labelMode !== 'none') {
            ...
            const labels = g.selectAll('.value-label')
                ...
                .attr('x', (d) => x(d.label)!)
                .attr('y', (d) => state.params.invertYAxis ? y(d.value) + 18 : y(d.value) - 12)
                .attr('opacity', 0)
