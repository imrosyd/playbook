...
function MillerLawChart({ lang }: { lang: any }) {
    const chunks = [
    ...
    return (
        <ChartFrame
            label={t(lang, 's1.decluttering101.millerChartLabel')}
            note={t(lang, 's1.decluttering101.millerChartNote')}
        >
            <svg viewBox={`0 0 ${w} ${h}`} className="w-full h-auto block p-4 bg-stone-50/30">
                ...
                <text x={pad.l + (87 / 100) * (w - pad.l - pad.r)} y={pad.t + 15}
                    fill="#f97316" fontSize={8}>{t(lang, 's1.decluttering101.millerLimitLabel')}</text>
                        ...
                        <g key={c.n}>
                            ...
                            {!c.ok && (
                                <g transform={`translate(${pad.l + bw + 8}, ${y + barH / 2 + 3})`}>
                                    <text fill="#ef4444" fontSize={7.5} fontWeight={500}>{t(lang, 's1.decluttering101.millerOverload')}</text>
                                </g>
                            )}
                        </g>
                    );
...
function DataInkComparisonChart({ lang }: { lang: any }) {
    type Mode = 'high' | 'low';
    ...
    return (
        <ChartFrame
            label={t(lang, 's1.decluttering101.dataInkChartLabel')}
            note={mode === 'high' ? t(lang, 's1.decluttering101.dataInkChartNoteHigh') : t(lang, 's1.decluttering101.dataInkChartNoteLow')}
        >
            <div className="space-y-4 p-4">
                <div className="flex justify-between items-center mb-2 px-2">
                    <div className="flex bg-stone-100/80 backdrop-blur-sm rounded-lg p-0.5 border border-stone-200">
                        {(['high', 'low'] as Mode[]).map(m => (
                            <button key={m} onClick={() => setMode(m)}
                                ...
                                {m === 'high' ? t(lang, 's1.decluttering101.btnHighNoise') : t(lang, 's1.decluttering101.btnTufte')}
                            </button>
                        ))}
                    </div>
                    <div className="flex gap-4">
                        <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[11px] font-medium text-stone-400">{t(lang, 's1.decluttering101.lblNoise')}</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                            <div className="w-2.5 h-2.5 rounded-full bg-emerald-500" />
                            <span className="text-[11px] font-medium text-stone-400">{t(lang, 's1.decluttering101.lblData')}</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex flex-wrap gap-x-6 gap-y-2 mt-2 px-2 border-t border-stone-100 pt-4">
                    <div className="flex items-center gap-2">
                        ...
                        <span className={`text-[11px] font-medium ${mode === 'high' ? 'text-rose-600 font-medium' : 'text-stone-400 line-through font-normal'}`}>{t(lang, 's1.decluttering101.lblZebra')}</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <Trash2 size={12} className={mode === 'high' ? 'text-rose-500' : 'text-stone-300'} />
                        <span className={`text-[11px] font-medium ${mode === 'high' ? 'text-rose-600 font-medium' : 'text-stone-400 line-through font-normal'}`}>{t(lang, 's1.decluttering101.lblBorder')}</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <Zap size={12} className="text-amber-500" />
                        <span className="text-[11px] font-medium text-stone-600">{mode === 'high' ? t(lang, 's1.decluttering101.lbl12Gridlines') : t(lang, 's1.decluttering101.lbl3Gridlines')}</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <Brain size={12} className="text-brand" />
                        <span className="text-[11px] font-medium text-emerald-600">{t(lang, 's1.decluttering101.lbl6DataBars')}</span>
                    </div>
                </div>
            </div>
        </ChartFrame>
...
function StrategicHighlightingLab({ lang }: { lang: any }) {
    const defaultData = [
    ...
    return (
        <ChartFrame
            label={t(lang, 's1.decluttering101.stratHighlightLabel')}
            note={isCleaned
                ? t(lang, 's1.decluttering101.stratHighlightNoteClean')
                : t(lang, 's1.decluttering101.stratHighlightNoteChaos')}
        >
            <div className="space-y-6 p-6">
                <div className="flex justify-between items-center mb-2">
                    <p className="text-[10px] font-bold text-stone-400 uppercase tracking-widest">
                        {t(lang, 's1.decluttering101.lblPerfByRegion')}
                    </p>
                    <button
                    ...
                    >
                        {isCleaned ? t(lang, 's1.decluttering101.btnResetChaos') : t(lang, 's1.decluttering101.btnCleanHighlight')}
                    </button>
                </div>
                            ...
                            >
                                {isCleaned && i === 0 && (
                                    <div className="absolute -top-10 left-1/2 -translate-x-1/2 whitespace-nowrap animate-bounce">
                                        <div className="bg-rose-600 text-white text-[8px] font-medium px-2 py-1 rounded shadow-lg">{t(lang, 's1.decluttering101.lblTarget')}</div>
        ...
        <LessonPage crossRefs={crossRefs}>
            <div className="space-y-8">
                ...
                <div className="space-y-4">
                    <p className="text-[15px] text-stone-600 leading-relaxed" dangerouslySetInnerHTML={{ __html: t(lang, 's1.decluttering101.intro1') }} />
                    <p className="text-[15px] text-stone-600 leading-relaxed" dangerouslySetInnerHTML={{ __html: t(lang, 's1.decluttering101.intro2') }} />
                    <p className="text-[15px] text-stone-600 leading-relaxed" dangerouslySetInnerHTML={{ __html: t(lang, 's1.decluttering101.intro3') }} />
                </div>

                {/* Miller's Law */}
                <MillerLawChart lang={lang} />
                ...
                <div className="space-y-3">
                    <div className="flex items-center gap-2 mb-2">
                        ...
                        <p className="text-[11px] font-bold text-stone-400 uppercase tracking-widest">
                            {t(lang, 's1.decluttering101.snFramework')}
                        </p>
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        {[
                            {
                                label: t(lang, 's1.decluttering101.snDataInkTitle'),
                                desc: t(lang, 's1.decluttering101.snDataInkDesc'),
                                color: 'emerald' as const,
                                action: t(lang, 's1.decluttering101.snDataInkAction'),
                            },
                            {
                                label: t(lang, 's1.decluttering101.snRedundantTitle'),
                                desc: t(lang, 's1.decluttering101.snRedundantDesc'),
                                color: 'amber' as const,
                                action: t(lang, 's1.decluttering101.snRedundantAction'),
                            },
                            {
                                label: t(lang, 's1.decluttering101.snNonDataTitle'),
                                desc: t(lang, 's1.decluttering101.snNonDataDesc'),
                                color: 'rose' as const,
                                action: t(lang, 's1.decluttering101.snNonDataAction'),
                            },
                        ].map((item) => (
                ...
                {/* Data-ink comparison */}
                <DataInkComparisonChart lang={lang} />

                {/* Strategic Highlighting Lab */}
                <StrategicHighlightingLab lang={lang} />
                ...
                <div className="bg-white rounded-xl border border-stone-200 p-5 shadow-sm">
                    <div className="relative">
                        ...
                        <p className="text-[11px] font-bold text-stone-400 uppercase tracking-wider mb-1">
                            {t(lang, 's1.decluttering101.liveDemoTitle')}
                        </p>
                        <p className="text-[13px] text-stone-500 mb-4">
                            {t(lang, 's1.decluttering101.liveDemoDesc')}
                        </p>
                        ...
                        <div className="flex items-center justify-center gap-3 mt-4">
                            <span className={`text-[13px] font-medium transition-colors ${!lowNoise ? 'text-stone-800' : 'text-stone-400'}`}>
                                {t(lang, 's1.decluttering101.btnChaos')}
                            </span>
                            <button
                            ...
                            <span className={`text-[13px] font-medium transition-colors ${lowNoise ? 'text-stone-800' : 'text-stone-400'}`}>
                                {t(lang, 's1.decluttering101.btnClean')}
                            </span>
                        </div>

                        <p className="text-center text-[12px] text-stone-400 mt-2">
                            {lowNoise
                                ? t(lang, 's1.decluttering101.demoCaptionClean')
                                : t(lang, 's1.decluttering101.demoCaptionChaos')}
                        </p>
                    </div>
                </div>
                ...
                <div className="rounded-xl bg-stone-50 border border-stone-200 p-5">
                    <p className="text-[11px] font-bold text-stone-400 uppercase tracking-wider mb-3">{t(lang, 's1.decluttering101.researchBasis')}</p>
                    <p className="text-[13px] text-stone-600 leading-relaxed mb-3" dangerouslySetInnerHTML={{ __html: t(lang, 's1.decluttering101.research1') }} />
                    <p className="text-[13px] text-stone-600 leading-relaxed" dangerouslySetInnerHTML={{ __html: t(lang, 's1.decluttering101.research2') }} />
                </div>
            </div>
        </LessonPage>
    );
