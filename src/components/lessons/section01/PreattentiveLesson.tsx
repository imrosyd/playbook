    ...
    return (
        <svg viewBox={`0 0 ${w} ${h}`} className="w-full">
            ...
            {[0, 150, 300, 450, 600].map(v => (
                <g key={v}>
                    ...
                    <text
                        ...
                        y={h - pad.b + 10}
                        fill="#a8a29e" fontSize={7} textAnchor="middle"
                    >{v}ms</text>
                </g>
            ))}
            {REACTION_DATA.map((d, i) => {
                ...
                return (
                    <g key={d.channel}>
                        <text x={pad.l - 6} y={y + barH / 2 + 4} fill="#78716c" fontSize={9} textAnchor="end">
                            {d.channel}
                        </text>
                        <rect x={pad.l} y={y} width={bw} height={barH} fill={d.color} rx={3} opacity={0.85} />
                        <text x={pad.l + bw + 4} y={y + barH / 2 + 4} fill="#78716c" fontSize={9}>
                            {d.ms}ms
                        </text>
                    </g>
                );
...
function GestaltDemo() {
    ...
    const circles = mode === 'grouped'
        ? [
            ...
            [30, 40], [50, 40], [30, 60], [50, 60],
            // Group B (right cluster)
            [130, 40], [150, 40], [130, 60], [150, 60],
            // Group C (far right)
            [220, 40], [240, 40], [220, 60], [240, 60],
        ]
        : [
            // Evenly spread
            [30, 50], [75, 50], [120, 50], [165, 50],
            [210, 50], [42, 50], [88, 50], [133, 50],
            [178, 50], [223, 50], [55, 50], [100, 50],
        ];
    ...
    return (
        <div className="space-y-3">
            ...
            <svg viewBox="0 0 280 100" className="w-full">
                {mode === 'grouped' && (
                    <>
                        <rect x={15} y={25} width={52} height={50} fill="#f0fdf4" rx={6} stroke="#bbf7d0" strokeWidth={1} />
                        <rect x={115} y={25} width={52} height={50} fill="#eff6ff" rx={6} stroke="#bfdbfe" strokeWidth={1} />
                        <rect x={205} y={25} width={52} height={50} fill="#fef3c7" rx={6} stroke="#fde68a" strokeWidth={1} />
                    </>
                )}
                {circles.map(([cx, cy], i) => (
                    <circle key={i} cx={cx} cy={cy} r={7} fill="#1c1917" opacity={0.75} />
                ))}
            </svg>
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < COLS * ROWS; i++) {
            ...
            if (withColor && i === TARGET_INDEX) {
                ...
                circle.setAttribute('fill', '#d6d3d1');
            }
        ...
        <LessonPage crossRefs={crossRefs}>
            <div className="space-y-8">
                ...
                <div className="bg-white rounded-xl border border-stone-200 p-5 shadow-sm space-y-3">
                    <p className="text-[11px] font-bold text-stone-400 uppercase tracking-wider">
                        Average detection time by visual channel (Treisman & Gelade, 1980)
                    </p>
                    <ReactionTimeChart />
                    <p className="text-[12px] text-stone-400 leading-relaxed">
                        Color hue detection (112ms) is 5× faster than shape conjunction search (580ms). Charts that use color to highlight a single point guarantee it will be seen before anything else — including the axis, title, or data source.
                    </p>
                </div>
                ...
                <div className="bg-white rounded-xl border border-stone-200 p-5 shadow-sm">
                    <div className="relative">
                        ...
                        <p className="text-center text-[12px] text-stone-400 mt-3 leading-relaxed">
                            {withColor
                                ? 'One red circle among 47 grey ones — found in under 200ms without searching. This is parallel pre-attentive detection.'
